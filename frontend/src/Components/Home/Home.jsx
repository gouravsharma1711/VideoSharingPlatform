  ...
  return (
    <div className="px-5 min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black mt-10">
      <div className="mb-8">
        ...
        <p className="text-gray-400">Discover the most popular content on our platform</p>
      </div>

      <VideosView videoData={videoData} lastVideoRef={lastVideoRef} />

      {loading && (
        <div className="text-center text-gray-300 mt-5">Loading more videos...</div>
      )}

      {!hasMore && (
        <div className="text-center text-gray-400 mt-5">No more videos to show</div>
      )}
    </div>
  );
}
